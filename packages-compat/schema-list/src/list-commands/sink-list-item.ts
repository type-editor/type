import {Fragment, type Node, type NodeRange, type NodeType, Slice} from '@type-editor-compat/model';
import type {EditorState, Transaction} from '@type-editor-compat/state';
import {ReplaceAroundStep} from '@type-editor-compat/transform';
...
export function sinkListItem(itemType: NodeType): Command {
    return (state: EditorState, dispatch?: (tr: Transaction) => void): boolean => {
        ...
        if (dispatch) {
            ...
            const nestedBefore: boolean = nodeBefore.lastChild && nodeBefore.lastChild.type === parent.type;
