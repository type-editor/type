...
import {type Attrs, Fragment, type Node, type NodeType, type ResolvedPos, Slice} from '@type-editor-compat/model';
import {type EditorState, Selection, type Transaction} from '@type-editor-compat/state';
import {canSplit} from '@type-editor-compat/transform';
...
export function splitListItem(itemType: NodeType, itemAttrs?: Attrs): Command {
    return (state: EditorState, dispatch?: (tr: Transaction) => void): boolean => {
        ...
        const transaction: Transaction = state.transaction.delete($from.pos, $to.pos);
 ...
 */
function splitNestedEmptyListItem(state: EditorState,
    ...
    const transaction: Transaction = state.transaction.replace(
        start,
 ...
 */
function findEmptyTextblock(transaction: Transaction, startPos: number): number {
    let position = -1;
