...
import { atBlockStart, findCutBefore } from './util/helpers';
 ...
 * ```
 */
export const joinTextblockBackward: Command = (state: PmEditorState,
...
function joinTextblocksAround(state: PmEditorState,
    ...
    const beforeNode: PmNode = $cut.nodeBefore;
    if (!beforeNode) {
        return false;
    }
