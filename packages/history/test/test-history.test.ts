...
import {ReplaceStep} from '@type-editor/transform';
import ist from "ist";
...
describe("history", () => {
  ...
  it("tracks multiple levels of history", () => {
    ...
    state = type(state, "a");
    state = type(state, "b");
    state = state.apply(state.transaction.insertText("c", 1));
  ...
  it("starts a new event when newGroupDelay elapses", () => {
    ...
    state = state.apply(state.transaction.insertText("a").setTime(1000));
    state = state.apply(state.transaction.insertText("b").setTime(1600));
    ist(undoDepth(state), 1);
    state = state.apply(state.transaction.insertText("c").setTime(2700));
    ist(undoDepth(state), 2);
    state = command(state, undo);
    state = state.apply(state.transaction.insertText("d").setTime(2800));
    ist(undoDepth(state), 2);
  });
  ...
  it("starts a new event for non-adjacent changes", () => {
    ...
    state = state.apply(state.transaction.insertText("y", 5));
    ist(undoDepth(state), 2);
  });

  it("doesn't get confused by non-replacement steps when checking adjacency", () => {
    let state = mkState(doc(p()), { newGroupDelay: 1000 });
    state = state.apply(state.transaction.insertText("x", 1).addMark(1, 2, schema.marks.em.create()));
    state = state.apply(state.transaction.insertText("y", 2).addMark(2, 3, schema.marks.em.create()));
    ist(undoDepth(state), 1);
  });
  ...
  it("allows changes that aren't part of the history", () => {
    ...
    state = state.apply(state.transaction.insertText("oops", 1).setMeta("addToHistory", false));
    state = state.apply(state.transaction.insertText("!", 10).setMeta("addToHistory", false));
    state = command(state, undo);
  ...
  it("doesn't get confused by an undo not adding any redo item", () => {
    let state = mkState();
    state = state.apply(state.transaction.insertText("foo"));
    state = state.apply(state.transaction.replaceWith(1, 4, schema.text("bar")).setMeta("addToHistory", false));
    state = command(state, undo);
    state = command(state, redo);
  ...
  function unsyncedComplex(state: EditorState, doCompress: boolean) {
    ...
    state = type(state, "!");
    state = state.apply(state.transaction.insertText("....", 1).setMeta("addToHistory", false));
    state = state.apply(state.transaction.split(3));
    ist(state.doc, doc(p(".."), p("..hello!")), eq);
    state = state.apply(state.transaction.split(2).setMeta("addToHistory", false));
  ...
  it("supports overlapping edits", () => {
    ...
    state = type(state, "hello");
    state = state.apply(closeHistory(state.transaction));
    state = state.apply(state.transaction.delete(1, 6));
    ist(state.doc, doc(p()), eq);
    state = command(state, undo);
  ...
  it("supports overlapping edits that aren't collapsed", () => {
    let state = mkState();
    state = state.apply(state.transaction.insertText("h", 1).setMeta("addToHistory", false));
  ...
  it("supports overlapping unsynced deletes", () => {
    ...
    state = state.apply(closeHistory(state.transaction));
    state = type(state, "hello");
    state = state.apply(state.transaction.delete(1, 8).setMeta("addToHistory", false));
    ist(state.doc, doc(p()), eq);
    state = command(state, undo);
    ist(state.doc, doc(p()), eq);
  });
  ...
  it("can go back and forth through history multiple times", () => {
    ...
    state = state.apply(state.transaction.insertText("zero ", 1));
    state = state.apply(closeHistory(state.transaction));
    state = state.apply(state.transaction.split(1));
    state = state.apply(state.transaction.setSelection(SelectionFactory.createTextSelection(state.doc, 1)));
  ...
  it("supports non-tracked changes next to tracked changes", () => {
    ...
    state = type(state, "o");
    state = state.apply(state.transaction.split(1));
    state = state.apply(state.transaction.insertText("zzz", 4).setMeta("addToHistory", false));
    state = command(state, undo);
  ...
  it("can go back and forth through history when preserving items", () => {
    ...
    state = type(state, " two");
    state = state.apply(closeHistory(state.transaction));
    state = state.apply(state.transaction.insertText("xxx", state.selection.head).setMeta("addToHistory", false));
    state = type(state, " three");
    state = state.apply(state.transaction.insertText("zero ", 1));
    state = state.apply(closeHistory(state.transaction));
    state = state.apply(state.transaction.split(1));
    state = state.apply(state.transaction.setSelection(SelectionFactory.createTextSelection(state.doc, 1)));
    state = type(state, "top");
    state = state.apply(state.transaction.insertText("yyy", 1).setMeta("addToHistory", false));
    for (let i = 0; i < 3; i++) {
  ...
  it("restores selection on undo", () => {
    ...
    state = type(state, "hi");
    state = state.apply(closeHistory(state.transaction));
    state = state.apply(state.transaction.setSelection(SelectionFactory.createTextSelection(state.doc, 1, 3)));
    let selection = state.selection;
    state = state.apply(state.transaction.replaceWith(selection.from, selection.to, schema.text("hello")));
  ...
  it("rebases selection on undo", () => {
    ...
    state = state.apply(closeHistory(state.transaction));
    state = state.apply(state.transaction.setSelection(SelectionFactory.createTextSelection(state.doc, 1, 3)));
    state = state.apply(state.transaction.insert(1, schema.text("hello")));
    state = state.apply(state.transaction.insert(1, schema.text("---")).setMeta("addToHistory", false));
    state = command(state, undo);
  ...
  it("supports querying for the undo and redo depth", () => {
    ...
    state = state.apply(state.transaction.insertText("b", 1).setMeta("addToHistory", false));
    ist(undoDepth(state), 1);
    ist(redoDepth(state), 0);
    state = command(state, undo);
  ...
  it("supports transactions with multiple steps", () => {
    let state = mkState();
    state = state.apply(state.transaction.insertText("a").insertText("b"));
    state = state.apply(state.transaction.insertText("c", 1));
    ist(state.doc, doc(p("cab")), eq);
    state = command(state, undo);
    ist(state.doc, doc(p("ab")), eq);
    ...
    let state = mkState(doc(p("x")), {
      plugins: [new Plugin({
        appendTransaction: (_trs, _old, state) => {
          if (state.doc.content.size == 4) return state.transaction.insert(1, schema.text("A"));
        }
      })]
    });
    state = state.apply(state.transaction.insert(2, schema.text("I")));
    ...
    let state = mkState(doc(p("x")), {
      plugins: [new Plugin({
        appendTransaction: (trs, _old, state) => {
          ...
          if (add) return state.transaction.insert(1, schema.text(add));
        }
      })]
    });
    state = state.apply(state.transaction.insert(2, schema.text("I")).setMeta("add", "A"));
    ist(state.doc, doc(p("AxI")), eq);
  ...
  it("doesn't close the history on appended transactions", () => {
    ...
    state = state.apply(state.transaction.insert(2, schema.text("R")).setMeta("add", "A"));
    state = state.apply(state.transaction.insert(3, schema.text("M")));
    state = command(state, undo);
    ist(state.doc, doc(p("x")), eq);
  });
  ...
  it("supports rebasing", () => {
    ...
    state = state.apply(state.transaction.step(rightStep));
  ...
  it("properly maps selection when rebasing", () => {
    ...
    state = state.apply(state.transaction.setSelection(SelectionFactory.createTextSelection(state.doc, 6, 13)));
    state = state.apply(state.transaction.delete(6, 13));
    let rebase = state.transaction.insert(6, schema.text("6789ABC")).insert(14, schema.text("E")).delete(6, 13)
