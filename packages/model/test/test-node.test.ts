...
import {a, blockquote, br, code, doc, em, eq, hr, img, li, p, schema, strong, ul} from './helper';
...
describe("Node", () => {
    ...
    describe("between", () => {
        function between(doc: Node, ...nodes: string[]) {
            let i = 0;
            doc.nodesBetween(doc.tag.a, doc.tag.b, (node, pos): undefined => {
                if (i == nodes.length)
    ...
    describe("automatic ID generation", () => {
        let schemaWithId: Schema;

        beforeEach(() => {
            schemaWithId = new Schema({
                nodes: {
                    doc: { content: "block+" },
        ...
        it("should generate a unique id when no id is provided", () => {
            const div1 = schemaWithId.nodes.div.create();
            const div2 = schemaWithId.nodes.div.create();

            ist(div1.attrs.id !== null);
            ist(div2.attrs.id !== null);
            ist(div1.attrs.id !== div2.attrs.id);
        });

        it("should generate a unique id when attrs is null", () => {
            const div = schemaWithId.nodes.div.create(null);

            ist(div.attrs.id !== null);
            ist(typeof div.attrs.id, "string");
            ist(div.attrs.id.length > 0);
        });

        it("should use provided id when id is explicitly given", () => {
            const customId = "my-custom-id";
            const div = schemaWithId.nodes.div.create({ id: customId });

            ist(div.attrs.id, customId);
        });

        it("should not generate id for nodes without id attribute in schema", () => {
            const para = schemaWithId.nodes.paragraph.create();

            ist(!("id" in para.attrs) || para.attrs.id === undefined);
        });

        it("should work with createChecked", () => {
            const div = schemaWithId.nodes.div.createChecked({ id: null });

            ist(div.attrs.id !== null);
            ist(typeof div.attrs.id, "string");
        });

        it("should work with createAndFill", () => {
            const div = schemaWithId.nodes.div.createAndFill();
