...
import {type Command, EditorState, type Transaction, SelectionFactory} from "@type-editor/state"
import {type Node, Schema} from "@type-editor/model";


import {blockquote, builders, doc, em, eq, img, p, schema} from "@type-editor/test-builder"
import ist from "ist"
import {SearchQuery} from "@src/SearchQuery";
import {findNext} from "@src/commands/find-next";
import {findNextNoWrap} from "@src/commands/find-next-no-wrap";
import {findPrev} from "@src/commands/find-prev";
import {findPrevNoWrap} from "@src/commands/find-prev-no-wrap";
import {replaceNext} from "@src/commands/replace-next";
import {replaceNextNoWrap} from "@src/commands/replace-next-no-wrap";
import {replaceCurrent} from "@src/commands/replace-current";
import {replaceAll} from "@src/commands/replace-all";
...
function mkState(query: Query, doc: Node) {
  ...
  return EditorState.create({
    doc,
    selection: a == null ? undefined : SelectionFactory.createTextSelection(doc, a, b),
    plugins: [searchPlugin({initialQuery: new SearchQuery(query), initialRange: query.range})]
  })
}
...
function testSelCommand(query: Query, doc: Node, command: Command) {
  ...
  ist(result, c != null)
  if (c != null) ist(JSON.stringify(state.selection), JSON.stringify(SelectionFactory.createTextSelection(doc, c, d)))
}
...
describe("search", () => {
  ...
  function footnoteSchema() {
    let footnoteSchema: Schema = new Schema({
