...
import {a, blockquote, br, doc, em, eq, hr, img, li, p, pre, schema, ul,} from '@type-editor/test-builder';
...
describe("EditorSelection.textSelectionBetween", () => {
    it("uses arguments when possible", () => {
        let d = doc(p("f<a>o<b>o"));
        let s = Selection.textSelectionBetween(d.resolve(d.tag.b), d.resolve(d.tag.a));
    ...
    it("will adjust when necessary", () => {
        let d = doc("<a>", p("foo"));
        let s = Selection.textSelectionBetween(d.resolve(d.tag.a), d.resolve(d.tag.a));
        ist(s.anchor, 1);
    });

    it("uses bias when adjusting", () => {
        let d = doc(p("foo"), "<a>", p("bar")), pos = d.resolve(d.tag.a);
        let sUp = Selection.textSelectionBetween(pos, pos, -1);
        ist(sUp.anchor, 4);
        let sDown = Selection.textSelectionBetween(pos, pos, 1);
        ist(sDown.anchor, 6);
    });
    ...
    it("will collapse towards the other argument", () => {
        let d = doc("<a>", p("foo"), "<b>");
        let s = Selection.textSelectionBetween(d.resolve(d.tag.a), d.resolve(d.tag.b));
        ist(s.anchor, 1);
        ist(s.head, 4);
        s = Selection.textSelectionBetween(d.resolve(d.tag.b), d.resolve(d.tag.a));
        ist(s.anchor, 4);
        ist(s.head, 1);
    });
});
