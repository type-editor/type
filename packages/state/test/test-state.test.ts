...
import {PluginKey} from "@src/plugin/PluginKey";
...
const transactionPlugin = new Plugin({
    ...
    appendTransaction(trs, _, state) {
        ...
        if (last && last.getMeta("append")) return state.transaction.insertText("A");
    }
});
...
describe("State", () => {
    ...
    it("applies transform transactions", () => {
        let state = EditorState.create({ schema });
        let newState = state.apply(state.transaction.insertText("hi"));
        ist(state.doc, doc(p()), eq);
    ...
    it("supports plugin fields", () => {
        ...
        let newState = state.apply(state.transaction).apply(state.transaction);
    ...
    it("can be serialized to JSON", () => {
        ...
        state = state.apply(state.transaction.setSelection(SelectionFactory.createTextSelection(state.doc, 3)));
    ...
    it("allows plugins to filter transactions", () => {
        ...
        let applied = state.applyTransaction(state.transaction.insertText("X"));
        ist(applied.state.doc, doc(p("X")), eq);
        ist(applied.transactions.length, 1);
        applied = state.applyTransaction(state.transaction.insertText("Y").setMeta("filtered", true));
    ...
    it("allows plugins to append transactions", () => {
        let state = EditorState.create({ plugins: [transactionPlugin], schema });
        let applied = state.applyTransaction(state.transaction.insertText("X").setMeta("append", true));
    ...
    it("stores a reference to a root transaction for appended transactions", () => {
        let state = EditorState.create({
            schema, plugins: [new Plugin({
                appendTransaction: (_trs, _oldState, newState) => newState.transaction.insertText("Y")
            })]
        });
        let { transactions } = state.applyTransaction(state.transaction.insertText("X"));
