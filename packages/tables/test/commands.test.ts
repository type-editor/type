...
describe('toggleHeader', () => {
  ...
  describe('useSelectedRowColumn option', () => {
    it('toggles entire selected row to headers', () =>
      test(
        doc(table(tr(c11, c11), tr(cCursor, c11))),
        toggleHeader('row', { useSelectedRowColumn: true }),
        doc(table(tr(c11, c11), tr(h11, h11))),
      ));

    it('toggles entire selected row from headers to cells', () =>
      test(
        doc(table(tr(c11, c11), tr(hCursor, h11))),
        toggleHeader('row', { useSelectedRowColumn: true }),
        doc(table(tr(c11, c11), tr(c11, c11))),
      ));

    it('toggles partial header row to all cells', () =>
      test(
        doc(table(tr(c11, c11), tr(hCursor, c11))),
        toggleHeader('row', { useSelectedRowColumn: true }),
        doc(table(tr(c11, c11), tr(c11, c11))),
      ));

    it('toggles entire selected column to headers', () =>
      test(
        doc(table(tr(c11, cCursor), tr(c11, c11))),
        toggleHeader('column', { useSelectedRowColumn: true }),
        doc(table(tr(c11, h11), tr(c11, h11))),
      ));

    it('toggles entire selected column from headers to cells', () =>
      test(
        doc(table(tr(h11, hCursor), tr(h11, h11))),
        toggleHeader('column', { useSelectedRowColumn: true }),
        doc(table(tr(h11, c11), tr(h11, c11))),
      ));

    it('toggles partial header column to all cells', () =>
      test(
        doc(table(tr(c11, hCursor), tr(c11, c11))),
        toggleHeader('column', { useSelectedRowColumn: true }),
        doc(table(tr(c11, c11), tr(c11, c11))),
      ));
  });

  describe('cell type toggling', () => {
    it('converts selected regular cells to header cells', () =>
      test(
        doc(table(tr(cCursor, c11), tr(c11, c11))),
        toggleHeader('cell'),
        doc(table(tr(hCursor, c11), tr(c11, c11))),
    ...
    it('converts selected header cells to regular cells', () =>
      test(
        doc(table(tr(hCursor, c11), tr(c11, c11))),
        toggleHeader('cell'),
        doc(table(tr(cCursor, c11), tr(c11, c11))),
    ...
    it('converts all selected cells to regular cells if any are headers', () =>
      test(
        doc(table(tr(th(p('x<anchor>')), cHead))),
        toggleHeader('cell'),
        doc(table(tr(c11, c11))),
      ));

    it('converts all selected cells to headers if none are headers', () =>
      test(
        doc(table(tr(cAnchor, cHead))),
        toggleHeader('cell'),
