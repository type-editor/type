import type {MapResult} from '@type-editor/editor-types';
 ...
 * ```
 *
 * @see IMapResult for the interface definition
 * @see StepMap.mapResult for the primary way to create MapResult instances
 */
export class PmMapResult implements MapResult {

    /**
     * The mapped position in the transformed document.
     * Stored privately and accessed through the `pos` getter.
     */
    private readonly _pos: number;

    /**
     * Bitwise deletion flags indicating what content was deleted.
     * Combination of DeletionInfo flags (BEFORE, AFTER, ACROSS, SIDE).
     * Stored privately and accessed through the `delInfo` getter.
     */
    private readonly _delInfo: number;

    /**
     * Recovery value for position recovery through mirrored transformations.
     * Null if no mirror relationship exists for this mapping.
     * Stored privately and accessed through the `recover` getter.
     */
    private readonly _recover: number | null;
     ...
     *
     * @param pos - The mapped version of the position in the transformed document.
     *              This represents where the original position ends up after applying
     *              the transformation(s). If the position was deleted, this will typically
     *              be the position where the deletion started.
     * @param delInfo - Bitwise combination of `DeletionInfo` flags indicating what content
     *                  was deleted relative to the position:
     *                  - `BEFORE` (1): Content before the position was deleted
     *                  - `AFTER` (2): Content after the position was deleted
     *                  - `ACROSS` (4): Content both before and after (spanning) was deleted
     *                  - `SIDE` (8): The position itself is on the deleted side
     *                  Flags can be combined using bitwise OR operations.
     * @param recover - Recovery value that can be used to map through mirrored transformations,
     *                  or `null` if no mirror relationship exists. When mapping through a sequence
     *                  of steps where some are inverses of others, this value enables position
     *                  recovery. This is essential for collaborative editing and undo/redo.
     *
     * @example
     * ```typescript
     ...
     *
     * @returns The mapped position as a non-negative integer.
     *
     * @example
     * ```typescript
     ...
     *
     * @returns A non-negative integer representing the bitwise combination of flags.
     *
     * @example
     * ```typescript
     ...
     *
     * @returns A recovery value (integer) if mirror recovery is possible, or `null` otherwise.
     *
     * @example
     * ```typescript
     ...
     * @see StepMap.recover for using recovery values
     */
    get recover(): number | null {
     ...
     */
    get deleted(): boolean {
        return (this.delInfo & DeletionInfo.SIDE) > 0;
    }
     ...
     *
     * @returns `true` if content before the position was deleted, `false` otherwise.
     *
     * @example
     * ```typescript
     * const deleteMap = new StepMap([5, 10, 0]); // Delete 10 chars at position 5
    ...
    get deletedBefore(): boolean {
        return (this.delInfo & (DeletionInfo.BEFORE | DeletionInfo.ACROSS)) > 0;
    }
     ...
     * adjusted after the transformation.
     *
     * @returns `true` if content after the position was deleted, `false` otherwise.
     *
     * @example
     * ```typescript
     * const deleteMap = new StepMap([10, 5, 0]); // Delete 5 chars at position 10
    ...
    get deletedAfter(): boolean {
        return (this.delInfo & (DeletionInfo.AFTER | DeletionInfo.ACROSS)) > 0;
    }
     ...
     *
     * @returns `true` if deletion spanned across the position, `false` otherwise.
     *
     * @example
     * ```typescript
    ...
    get deletedAcross(): boolean {
        return (this.delInfo & DeletionInfo.ACROSS) > 0;
    }
}
