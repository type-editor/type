...
import {type Node, Schema, Slice} from '@type-editor/model';
import {eq, schema as baseSchema} from '@type-editor/test-builder';
import ist from "ist";
import {canSplit} from "@src/change-helper/can-split";
...
describe("canSplit", () => {
    ...
    it("doesn't return true when the split-off content doesn't fit in the given node type", () => {
        let s: Schema = new Schema({
            nodes: schema.spec.nodes.addBefore("heading", "title", { content: "text*" })
