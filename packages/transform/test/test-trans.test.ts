...
import {testTransform} from "./trans";
import {
    type Attrs,
    Fragment,
    type Mark,
...
import {findWrapping} from "@src/change-helper/find-wrapping";
import {liftTarget} from "@src/block-changes/lift-target";
...
describe("Transform", () => {
    ...
    const linebreakSchema: Schema = new Schema({
    ...
    describe("setBlockType", () => {
        ...
        it("can base attributes on previous attributes", () =>
            type(doc("<a>", h1("a"), p("b"), "<b>"),
                doc(h2("a"), h1("b")),
                "heading", node => ({ level: (node.attrs.level as number || 0) + 1 })));

        describe("fast path (applySinglePassTransformation)", () => {
            it("uses fast path for simple block changes without marks", () =>
                type(doc(p("<a>hello"), p("world<b>")),
                    doc(h1("hello"), h1("world")),
                    "heading", { level: 1 }));

            it("uses fast path when converting paragraph to heading (compatible marks)", () =>
                type(doc(p("<a>hello ", em("world"))),
                    doc(h1("hello ", em("world"))),
                    "heading", { level: 1 }));

            it("uses fast path for many blocks at once", () =>
                type(doc(p("<a>one"), p("two"), p("three"), p("four"), p("five<b>")),
                    doc(h1("one"), h1("two"), h1("three"), h1("four"), h1("five")),
                    "heading", { level: 1 }));

            it("uses fast path when converting headings to paragraphs", () =>
                type(doc(h1("<a>title"), h2("subtitle<b>")),
                    doc(p("title"), p("subtitle")),
                    "paragraph"));

            it("uses fast path with nested compatible structures", () =>
                type(doc(blockquote(p("<a>one"), p("two<b>"))),
                    doc(blockquote(h1("one"), h1("two"))),
                    "heading", { level: 1 }));

            it("uses fast path and maintains position integrity", () =>
                // Testing reverse-order processing doesn't break positions
                type(doc(p("<a>a"), p("b"), p("c"), p("d"), p("e"), p("f"), p("g"), p("h"), p("i"), p("j<b>")),
                    doc(h1("a"), h1("b"), h1("c"), h1("d"), h1("e"), h1("f"), h1("g"), h1("h"), h1("i"), h1("j")),
                    "heading", { level: 1 }));
        });

        describe("complex path (applyComplexConversions)", () => {
            it("uses complex path when clearing incompatible marks", () =>
                type(doc(p("<a>hello ", em("world"))),
                    doc(pre("hello world")),
                    "code_block"));

            it("uses complex path when content is incompatible", () =>
                type(doc(p("<a>text", img(), "more")),
                    doc(pre("textmore")),
                    "code_block"));

            it("uses complex path when clearing newlines from pre to paragraph", () =>
                type(doc(pre("<a>line1\nline2\nline3")),
                    doc(p("line1 line2 line3")),
                    "paragraph"));

            it("uses complex path when converting newlines to linebreaks", () => {
                type(lb.doc(lb.pre("<a>one\ntwo\nthree")),
                    lb.doc(lb.p("one", lb.br(), "two", lb.br(), "three")),
                    "paragraph");
            });

            it("uses complex path when converting linebreaks to newlines", () => {
                type(lb.doc(lb.p("<a>one", lb.br(), "two", lb.br(), "three")),
                    lb.doc(lb.pre("one\ntwo\nthree")),
                    "code_block");
            });
        ...
        describe("mixed scenarios", () => {
            it("handles mix of simple and complex blocks correctly", () => {
                // First block needs complex handling (has marks to clear)
                // Second block uses fast path (no special handling needed)
                type(doc(p("<a>hello ", em("world")), p("simple<b>")),
                    doc(pre("hello world"), pre("simple")),
                    "code_block");
            });

            it("handles partially compatible range with constraints", () => {
                // Some blocks can be changed, some can't due to constraints
                type(doc(p("<a>can change"), ul(li(p("cannot change"))), p("can change<b>")),
                    doc(h1("can change"), ul(li(p("cannot change"))), h1("can change")),
                    "heading", { level: 1 });
            });

            it("processes blocks in correct order with complex conversions", () => {
                // Multiple blocks needing complex handling
                type(doc(p("<a>one ", em("a")), p("two ", em("b")), p("three ", em("c<b>"))),
                    doc(pre("one a"), pre("two b"), pre("three c")),
                    "code_block");
            });
        });
    });
    ...
    describe("replace", () => {
        function repl(doc: Node, source: Node | Slice | null, expect: Node) {
            let slice = !source ? Slice.empty : source instanceof Slice ? source
                : (source as Node).slice(tag(source as Node, "a"), tag(source as Node, "b"));
            testTransform(new Transform(doc).replace(tag(doc, "a"), tag$(doc, "b") || tag(doc, "a"), slice as Slice), expect);
        }
        ...
        let hbSchema: Schema = new Schema({
        ...
        it("can split a selfPos and place its children in different parents", () => {
    ...
    describe("replaceRange", () => {
        function repl(doc: Node, source: Node, expect: Node) {
            ...
            testTransform(new Transform(doc).replaceRange(tag(doc, "a"), tag$(doc, "b") || tag(doc, "a"), slice as Slice), expect);
        }
    ...
    describe("setDocAttribute", () => {
        let schema: Schema = new Schema({
            nodes: {
                doc: { content: "text*", attrs: { meta: { default: null } } },
                text: {}
            },
        });
