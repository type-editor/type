...
import ist from "ist";
import {blockquote, builders, doc, h1, li, p, schema, ul} from '@type-editor/test-builder';
import {
    liftTarget,
    ReplaceAroundStep,
    ReplaceStep,
...
import {Fragment, type Node, NodeRange, Schema, Slice} from '@type-editor/model';
import {Decoration, DecorationSet} from '@type-editor/decoration';
import type {DecorationSpec, NodeDecorationOptions, PmDecoration} from '@type-editor/editor-types';

let widget = document.createElement("button");

type DecoSpec = PmDecoration | DecorationSpec;

function make(d: DecoSpec) {
  if ((d as PmDecoration).type) return d as PmDecoration;
  if ((d as any).pos != null) return Decoration.widget((d as { pos: number; }).pos, widget, d as any);
  if ((d as any).node) return Decoration.node((d as any).from, (d as any).to, (d as any).attrs || {}, d as NodeDecorationOptions);
...
function buildMap(doc: Node, ...decorations: (DecoSpec | ((tr: TransformDocument) => TransformDocument))[]) {
...
class TestDecorationSource extends DecorationSet {

    public removeOverlap(spans: ReadonlyArray<PmDecoration>): Array<PmDecoration> {
        return super.removeOverlap(spans);
    }
}

const decorationSourceTestInstance = new TestDecorationSource([], []);
...
describe("DecorationSet", () => {
  ...
  describe("map", () => {
    ...
    it("calls onRemove when dropping decorations", () => {
      ...
      set.map(tr.mapping, tr.doc, { onRemove: o => dropped.push(o.name.toString()) });
      ist(JSON.stringify(dropped), '["a"]');
      let tr2 = new Transform(d).delete(0, d.content.size), dropped2: string[] = [];
      set.map(tr2.mapping, tr2.doc, { onRemove: o => dropped2.push(o.name.toString()) });
    ...
    it("correctly offsets a deep structure", () => {
      ...
      d.descendants((node, pos): undefined => {
...
describe("removeOverlap", () => {
  it("returns the original array when there is no overlap", () => {
    ...
    ist(decorationSourceTestInstance.removeOverlap(decs), decs);
  });

  it("splits a partially overlapping decoration", () => {
    let decs = [make({ from: 1, to: 2 }), make({ from: 1, to: 4 }), make({ from: 3, to: 4 })];
    ist(arrayStr(decorationSourceTestInstance.removeOverlap(decs)), "1-2, 1-2, 2-3, 3-4, 3-4");
  });

  it("splits a decoration that spans multiple widgets", () => {
    let decs = [make({ from: 1, to: 5 }), make({ pos: 2 }), make({ pos: 3 })];
    ist(arrayStr(decorationSourceTestInstance.removeOverlap(decs)), "1-2, 2-2, 2-3, 3-3, 3-5");
  });

  it("correctly splits overlapping inline decorations", () => {
    let decs = [make({ from: 0, to: 6 }), make({ from: 1, to: 4 }), make({ from: 3, to: 5 })];
    ist(arrayStr(decorationSourceTestInstance.removeOverlap(decs)), "0-1, 1-3, 1-3, 3-4, 3-4, 3-4, 4-5, 4-5, 5-6");
  });
});
