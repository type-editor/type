...
import ist from "ist";
import {a, blockquote, br, doc, em, eq, h1, img as img_, p, pre, strong} from '@type-editor/test-builder';
import {Step} from '@type-editor/transform';
import {findTextNode, flush, tempEditor} from './view';
import {EditorState, SelectionFactory, Transaction} from "@type-editor/state";
...
describe("DOM change", () => {
  ...
  it("respects stored marks", () => {
    let view = tempEditor({ doc: doc(p("hello")) });
    view.dispatch(view.state.transaction.addStoredMark(view.state.schema.marks.em.create()));
  ...
  it("detects an enter press", () => {
    ...
    let view = tempEditor({
      ...
      handleKeyDown: (_view, event) => {
        if (event.keyCode == 13) return enterPressed = true; 
      }
    });
  ...
  it("resolves ambiguous text input", () => {
    ...
    view.dispatch(view.state.transaction.addStoredMark(view.state.schema.marks.strong.create()));
  ...
  it("recognizes a mark change as such", () => {
    let view = tempEditor({
      doc: doc(p("one")), dispatchTransaction(tr) {
  ...
  it("preserves marks on deletion", () => {
    ...
    let emNode = view.dom.querySelector("em")!;
    emNode.textContent = "";
    setSel(emNode, 0);
    flush(view);
    view.dispatch(view.state.transaction.insertText("y"));
  ...
  it("works when a node's contentDOM is deleted", () => {
    ...
    // innerText is not implemented in jsdom so use textContent instead
    view.dom.querySelector("code").textContent = "";
    view.dispatchEvent({ type: "input" } as Event);
    flush(view);
  ...
  it("maps input to coordsAtPos through pending changes", () => {
    ...
    view.dispatchEvent({ type: "input" } as Event);
    view.dispatch(view.state.transaction.insertText("more text"));
  ...
  it("doesn't confuse backspace with delete", () => {
    ...
    let view: PmEditorView = tempEditor({
      doc: doc(p("a<a>a")),
    ...
    view.input.lastKey = KEY_BACKSPACE;
  ...
  it("doesn't confuse backspace with delete", () => {
    let steps: ReadonlyArray<Step>, view = tempEditor({
      doc: doc(p("a<a>a")),
      dispatchTransaction(tr) {
        steps = tr.steps
        view.updateState(view.state.apply(tr as Transaction))
      }
    })
  ...
  it("creates a step that covers the entire selection for partially-matching replacement", () => {
    ...
    view.dispatch(view.state.transaction.setSelection(SelectionFactory.createTextSelection(view.state.doc, 7, 12)));
