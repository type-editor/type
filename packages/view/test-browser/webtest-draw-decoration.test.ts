...
import {blockquote, doc, em, h1, hr, img, p, schema, strong} from '@type-editor/test-builder';
import {type EditorState, Plugin, SelectionFactory} from '@type-editor/state';
import {Schema} from '@type-editor/model';
import {Decoration, DecorationSet} from '@type-editor/decoration';
import {type PmDecoration, type DecorationSource, type PmEditorState} from '@type-editor/editor-types';
import {type EditorView} from '@src/EditorView';
import {tempEditor} from './view';
import ist from "ist";
import {Mapping} from "@type-editor/transform";

function make(str: string | PmDecoration): PmDecoration {
...
function decoPlugin(decos: readonly (string | PmDecoration)[]) {
  return new Plugin({
    state: {
      init(config) { return DecorationSet.create(config.doc!, decos.map(make)); },
      apply(tr, set, state) {
        if (tr.docChanged) set = set.map(tr.mapping as Mapping, tr.doc);
    ...
    props: {
      decorations(this: Plugin, state: PmEditorState) { return this.getState(state as EditorState); }
    }
  });
}

function updateDeco(view: EditorView, add: readonly PmDecoration[] | null, remove?: readonly PmDecoration[]) {
  view.dispatch(view.state.transaction.setMeta("updateDecorations", { add, remove }));
}
...
describe("Decoration drawing", () => {
  ...
  it("calls widget destroy methods", () => {
    ...
    view.dispatch(view.state.transaction.delete(1, 4));
    ist(destroyed);
  });
  ...
  it("doesn't redraw when irrelevant content changes", () => {
    ...
    let para2 = view.dom.lastChild;
    view.dispatch(view.state.transaction.delete(2, 3));
    view.dispatch(view.state.transaction.delete(2, 3));
    ist(view.dom.lastChild, para2);
  });
  ...
  it("can remove the class for part of a text node1", () => {
    let dec = make("2-4-foo");
    let view = tempEditor({
      doc: doc(p("abcd")),
      plugins: [decoPlugin([dec])]
  ...
  it("can update a text node around a widget", () => {
    ...
    view.dispatch(view.state.transaction.delete(1, 2));
    ist(view.dom.querySelectorAll("button").length, 1);
  ...
  it("doesn't redraw nodes when a widget before them is replaced", () => {
    ...
    view.dispatch(view.state.transaction.setMeta("updateDecorations", { add: [make("3-widget")], remove: [w0] })
      .insertText("c", 5));
  ...
  it("doesn't redraw widgets with matching keys", () => {
    ...
    view.dispatch(view.state.transaction.insertText("!", 2, 2));
  ...
  it("doesn't get confused by split text nodes", () => {
    let view = tempEditor({
      doc: doc(p("abab")), decorations(state) {
    ...
    view.dispatch(view.state.transaction.setSelection(SelectionFactory.createTextSelection(view.state.doc, 5)));
    ist(view.dom.textContent, "abab");
  });

  function thingSchema(): Schema {
    return new Schema({
      nodes: {
