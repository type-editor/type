...
import {doc, h1, hr, p, pre, schema, strong} from '@type-editor/test-builder';
...
describe("EditorView draw", () => {
  it("updates the DOM", () => {
    ...
    view.dispatch(view.state.transaction.insertText("bar"));
  ...
  it("doesn't redraw nodes after changes", () => {
    ...
    view.dispatch(view.state.transaction.insertText("!"));
  ...
  it("doesn't redraw nodes before changes", () => {
    ...
    let oldP = view.dom.querySelector("p");
    view.dispatch(view.state.transaction.insertText("!", 2));
    ist(view.dom.querySelector("p"), oldP);
  });
  ...
  it("doesn't redraw siblings of a split node", () => {
    ...
    let oldP = view.dom.querySelector("p");
    let oldPre = view.dom.querySelector("pre");
    view.dispatch(view.state.transaction.split(8));
    ist(view.dom.querySelector("p"), oldP);
    ist(view.dom.querySelector("pre"), oldPre);
  });
  ...
  it("doesn't redraw siblings of a joined node", () => {
    ...
    let oldP = view.dom.querySelector("p");
    let oldPre = view.dom.querySelector("pre");
    view.dispatch(view.state.transaction.join(10));
    ist(view.dom.querySelector("p"), oldP);
    ist(view.dom.querySelector("pre"), oldPre);
  });
  ...
  it("doesn't redraw after a big deletion", () => {
    ...
    view.dispatch(view.state.transaction.delete(2, 14));
  ...
  it("doesn't redraw following paragraphs when a paragraph is split", () => {
    ...
    view.dispatch(view.state.transaction.split(3));
  ...
  it("doesn't greedily match nodes that have another match", () => {
    ...
    view.dispatch(view.state.transaction.split(2));
  ...
  it("creates and destroys plugin views", () => {
    ...
    view.dispatch(view.state.transaction.insertText("u"));
    view.destroy();
  ...
  it("doesn't get confused by merged nodes", () => {
    ...
    view.dispatch(view.state.transaction.removeMark(1, 4, schema.marks.strong));
  ...
  it("doesn't redraw too much when marks are present", () => {
    ...
    view.dispatch(view.state.transaction.replaceWith(0, 8, newParagraphs));
