...
import {a, doc, p, schema} from '@type-editor/test-builder';
import ist from "ist";
...
describe("EditorView.endOfTextblock", () => {
  ...
  it("works for virtual motion when in a mark", () => {
    let view = tempEditor({
      doc: doc(p(a("fo<a>o "), new Array(50).join("foo "), a("fo<b>o "),
        new Array(50).join("foo "), a("fo<c>o")))
    });
    ist(view.endOfTextblock("up"));
    ist(!view.endOfTextblock("down"));
    view.dispatch(view.state.transaction.setSelection(SelectionFactory.createTextSelection(view.state.doc, view.state.doc.tag.b)));
    ist(!view.endOfTextblock("up"));
    ist(!view.endOfTextblock("down"));
    view.dispatch(view.state.transaction.setSelection(SelectionFactory.createTextSelection(view.state.doc, view.state.doc.tag.c)));
    ist(!view.endOfTextblock("up"));
    ist(view.endOfTextblock("down"));
  });
  ...
  it("works in a cursor wrapper", () => {
    ...
    view.dispatch(view.state.transaction.setStoredMarks([schema.marks.em.create()]));
    ist(!view.endOfTextblock("backward"));
  });
