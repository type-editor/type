...
import {blockquote, br, doc, p} from '@type-editor/test-builder';
import {Plugin} from '@type-editor/state';
import {Decoration, DecorationSet} from '@type-editor/decoration';
import {type PmDecoration, type ViewMutationRecord} from '@type-editor/editor-types';
import ist from "ist";
import {flush, tempEditor} from './view';
...
describe("nodeViews prop", () => {
  ...
  it("can override drawing of a node's content", () => {
    ...
    view.dispatch(view.state.transaction.insertText("a"));
  ...
  it("has its destroy method called", () => {
    ...
    ist(!destroyed);
    view.dispatch(view.state.transaction.delete(3, 5));
    ist(destroyed);
  });
  ...
  it("has access to outer decorations", () => {
    ...
    let view = tempEditor({
      ...
      nodeViews: {
        hard_break(_n, _v, _p, deco) {
          ...
          function update(deco: readonly PmDecoration[]) {
            dom.textContent = deco.length ? deco[0].spec.name.toString() : "[]";
          }
          update(deco);
    ...
    view.dispatch(view.state.transaction.setMeta("setDeco", "foo"));
    ist(view.dom.querySelector("var")!.textContent, "foo");
    view.dispatch(view.state.transaction.setMeta("setDeco", "bar"));
    ...
    let view = tempEditor({
      ...
      nodeViews: {
        paragraph(node) {
          let dom = document.createElement("p");
          return {
            dom, contentDOM: dom, update(node_, _, innerDecoSet) {
