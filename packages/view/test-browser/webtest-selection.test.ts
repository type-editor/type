...
import {blockquote, br, code, code_block, doc, em, hr, img as img_, li, p, strong, ul,} from '@type-editor/test-builder';
import {findTextNode, flush, tempEditor,} from './view';
...
function allPositions(doc: Node) {
  let found: number[] = [];
  function scan(node: Node, start: number) {
    if (node.isTextblock) {
...
function setDOMSel(node: DOMNode, offset: number) {
...
function getSel() {
  ...
  while (node && node.nodeType != TEXT_NODE) {
    ...
    else if (before) { node = before; offset = node.nodeType == TEXT_NODE ? node.nodeValue!.length : node.childNodes.length; }
    else break;
  }
  return { node: node, offset: offset };
}

function setSel(view: EditorView, sel: number | PmSelection) {
  if (typeof sel == "number") sel = Selection.near(view.state.doc.resolve(sel));
  view.dispatch(view.state.transaction.setSelection(sel));
}
...
function createInputEvent(type: string, inputType: string) {
  return new InputEvent(type, {inputType});
}
...
describe("EditorView", () => {
  it("can read the DOM selection", () => {
    ...
    function test(node: DOMNode, offset: number, expected: number) {
  ...
  it("syncs the DOM selection with the editor selection", () => {
    ...
    let view = tempEditor({ doc: doc(p("one"), hr(), blockquote(p("two"))) });
    function test(pos: number, node: DOMNode, offset: number) {
      setSel(view, pos);
      let sel = getSel();
      ist(sel.node, node);
  ...
  it("returns correct screen coordinates for wrapped lines", () => {
    ...
    for (let i = 0; i < 100; i++) {
      view.dispatch(view.state.transaction.insertText("a bc de fg h"));
      pos += 12;
  ...
  it("makes arrow motion go through selectable inline nodes", async () => {
    ...
    await keyboardEvent.keyboard('{ArrowRight}')
    await keyboardEvent.keyboard('{/ArrowRight}')
    ist(view.state.selection.from, 4)
    await keyboardEvent.keyboard('{ArrowRight}')
    await keyboardEvent.keyboard('{/ArrowRight}')
    ...
    await keyboardEvent.keyboard('{ArrowLeft}')
    await keyboardEvent.keyboard('{/ArrowLeft}')
    ist(view.state.selection.from, 4)
    await keyboardEvent.keyboard('{ArrowLeft}')
    await keyboardEvent.keyboard('{/ArrowLeft}')
  ...
  it("makes arrow motion go through selectable block nodes", async () => {
    ...
    await keyboardEvent.keyboard('{ArrowDown}')
    await keyboardEvent.keyboard('{/ArrowDown}')
    ist(view.state.selection.from, 7);
    setSel(view, 11);
    await keyboardEvent.keyboard('{ArrowUp}')
    await keyboardEvent.keyboard('{/ArrowUp}')
    ist(view.state.selection.from, 7);
  });
  ...
  it("allows moving directly from an inline node to a block node", async () => {
    ...
    setSel(view, SelectionFactory.createNodeSelection(view.state.doc, 4));
    await keyboardEvent.keyboard('{ArrowDown}')
    await keyboardEvent.keyboard('{/ArrowDown}')
    ist(view.state.selection.from, 6);
    setSel(view, SelectionFactory.createNodeSelection(view.state.doc, 8));
    await keyboardEvent.keyboard('{ArrowUp}')
    await keyboardEvent.keyboard('{/ArrowUp}')
    ist(view.state.selection.from, 6);
  });
